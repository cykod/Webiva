...
for file in config = %w{ workling.yml } do
  FileUtils.cp(File.join(plugin_root, 'config', file), File.join(Rails.root, 'config'))
end

# scripts
for file in script = %w{ workling_starling_client workling_client bj_invoker.rb starling_status.rb } do
  FileUtils.cp File.join(plugin_root, 'script', file), File.join(Rails.root, 'script')
  FileUtils.chmod 0755, File.join(Rails.root, 'script', file)
end
