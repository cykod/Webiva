...
module SimpleCaptchaImage #:nodoc
  ...
  def generate_simple_captcha_image(options={})  #:nodoc
    ...
    @simple_captcha_image_options = {
      ...
      :color => 'darkblue',
      :distortion => SimpleCaptchaImage.distortion(options[:distortion]),
      :image_style => SimpleCaptchaImage.image_style(options[:image_style])
    }
