...
class Content::CoreFeature::EmailTargetConnect <  Content::Feature
   ...
   class EmailTargetConnectOptions < HashModel
    ...
    def matched_fields=(val)
      int_val = {}
      val.each { |key,fld| int_val[key.to_i] = fld }
    ...
    def email_target_connect_update
      if update_data = self.email_target_connect
        ...
        connected_fields.each do |field|
          if field[1] == 'data'
            target.options ||= {}
            target.options[field[0].to_sym] = self.respond_to?("#{field[0]}_display") ? self.send("#{field[0]}_display") : self.send(field[0])
          else
            ...
            obj.send("#{usr_field}=",self.send(field[0])) if obj.send(usr_field).blank?
          end
        end
