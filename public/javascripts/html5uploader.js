...
function uploader(place, status, targetPHP, show, uploadForm) {
	...
	upload = function(file) {
          params = uploadForm ? '&' + $j('#' + uploadForm).serialize() : '';
          alert( params );
		...
		if(window.FileReader) { 
			...
			this.loadEnd = function() {
				...
				xhr.open('POST', targetPHP+'?up=true' + params, true);
				var boundary = 'xxxxxxxxx';
	 			var body = '--' + boundary + "\r\n";  
				body += "Content-Disposition: form-data; name=upload_file_filename; filename=" + file.name + "\r\n";  
				...
				// Chrome 7 sends data but you must use the base64_decode on the PHP side
					...
					xhr.open('POST', targetPHP+'?up=true&base64=true' + params, true);
