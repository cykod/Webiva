...
SortableTree.Node = Class.create({
  ...
  onDrop: function(drag, drop, event) {
    var tree_drag = this.tree.find(drag);
    if (tree_drag) drag = tree_drag;

    var tree_drop = this.tree.find (drop);
    if (tree_drop) drop = tree_drop;

    if (tree_drag && tree_drop) {
    ...
    if(this.options.onDrop) this.options.onDrop(drag, drop, event,tree_drag && tree_drop,this.dropPosition);
  },
